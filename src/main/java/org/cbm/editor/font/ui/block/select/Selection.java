...
public class Selection
{
	...
	private static final Stroke SELECTION_STROKE = new BasicStroke(1, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0,
			new float[] {
					4, 4
			}, 0);
	private static final Stroke SELECTION_BACKGROUND_STROKE = new BasicStroke(1, BasicStroke.CAP_ROUND,
	...
	private boolean isInSelector(final Point positionInComponent, final Rectangle r, final Selector selector)
	{
		...
		return (positionInComponent.x >= (selectorPoint.x - 5)) && (positionInComponent.y >= (selectorPoint.y - 5))
				&& (positionInComponent.x <= (selectorPoint.x + 5)) && (positionInComponent.y <= (selectorPoint.y + 5));
	}
	...
	public boolean updateSelection(final Point positionInComponent, final Selector selector)
	{
		...
		if (selection.equals(this.rectangle))
		{
			return false;
		}

		this.rectangle = selection;
