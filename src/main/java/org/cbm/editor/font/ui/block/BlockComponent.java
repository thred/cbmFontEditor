...
public class BlockComponent extends JComponent implements MouseListener, MouseMotionListener, FocusListener,
		KeyListener
{
	...
	{
		return new Point((int) Math.floor(positionInComponent.x / zoom), (int) Math.floor(positionInComponent.y / zoom));
	}
	...
	public Rectangle convertFromPixelToCharacter(Rectangle rectangleInPixel)
	{
		...
		int h = (int) Math.ceil((double) (rectangleInPixel.y + rectangleInPixel.height) / block.getCharacterHeight())
				- y;
	...
	void updatePreferredSize()
	{
		...
		Dimension preferredSize = new Dimension(
				(int) ((rootLayer.getWidthInPixel() + (2 * rootLayer.getOverscanInPixel())) * zoom),
