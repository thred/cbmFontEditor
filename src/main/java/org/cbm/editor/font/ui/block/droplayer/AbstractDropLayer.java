...
public abstract class AbstractDropLayer extends AbstractLayer implements DropLayer, MouseListener, MouseMotionListener
{
	...
	private static final Stroke DROP_STROKE = new BasicStroke(1, BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND, 0,
			new float[] {
					4, 4
			}, 0);
	private static final Stroke DROP_BACKGROUND_STROKE = new BasicStroke(1, BasicStroke.CAP_ROUND,
		...
		if (this.blockSelection != blockSelection)
		{
			...
			if (blockSelection != null)
			{
				...
				if (center)
				{
					...
					Point visibleCenterInPixel = component.convertFromComponentToPixel(component.transfer(new Point(
							visibleRectangle.x + (visibleRectangle.width / 2), visibleRectangle.y
	...
	public void mousePressed(MouseEvent e)
	{
		...
		if ((e.getModifiers() & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK)
		{
			...
			{
				referenceInPixel = new Point(positionInPixel.x - rectangleInPixel.x, positionInPixel.y
						- rectangleInPixel.y);
				e.consume();
			}
			else
