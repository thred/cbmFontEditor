...
public class Colors
{
	...
	{
		final Paint paint = new LinearGradientPaint(0, 0, 0, height, new float[] {
				0f, 0.03f, 0.97f, 1f
		}, new Color[] {
				Colors.darker(highlight, 0.1f), Colors.darker(highlight, 0.9f), color, color
		});
	...
	public static void backgroundBottom(Graphics graphics, Color color, int width, int height)
	{
		final Paint paint = new LinearGradientPaint(0, 0, 0, height, new float[] {
				...
				Colors.darker(color, 0.7f), color, Colors.darker(color, 0.9f), Colors.darker(color, 0.1f)
		});
