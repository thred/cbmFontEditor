...
public class VideoEmulationBlockImage extends AbstractBlockImage
{
    ...
    public VideoEmulationBlockImage(Font font, Block block)
    {
        super(font, block);
    }

    public VideoEmulationBlockImage(Font font, Block block, Rectangle clip)
    {
    ...
    protected Image render(Font font, Block block, Rectangle clip, double zoom)
    {
        ...
        final Carrier lumaCarrier =
            this.lumaCarrier = Carrier.create(this.lumaCarrier, (int) (lumaPerPixel * width) + 1);
        final Carrier chromaCarrier =
            this.chromaCarrier = Carrier.create(this.chromaCarrier, (int) (chromaPerPixel * width) + 1);
        ...
        for (int i = 0; i < 2; i += 1)
        {
            ...
            for (int y = 0; y < height; y += 1)
            {
                ...
                //final float lumaMultiplier = (y % 2 == 0) ? 1f : 0.75f;
                ...
                for (int lCarrierX = 0; lCarrierX < lumaCarrier.getSourceLength(); lCarrierX += 1)
                {
                    ...
                    final float target = getL(font, block, sourceX, sourceY);// * lumaMultiplier;
