...
public class EditBlockLayer extends AbstractBlockLayer implements MouseListener, MouseMotionListener
{
    ...
    public void mousePressed(MouseEvent e)
    {
        ...
        if ((modifiers & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK)
        {
            ...
            {
                draw = new Draw(fontAdapter.getFont(), getBlock(),
                    getComponent().convertFromComponentToPixel(positionInComponent));
                e.consume();
                return;
            }
        }
    ...
    public void mouseReleased(MouseEvent e)
    {
        ...
        if ((e.getModifiers() & InputEvent.BUTTON1_MASK) == InputEvent.BUTTON1_MASK)
        {
            ...
            else if (getTool() == Tool.FILL)
            {
                ...
                if (block.getCharacter(position.x / block.getCharacterWidth(),
                    position.y / block.getCharacterHeight()) < 0)
                {
                    return;
                }

                Registry.execute(new FillEdit(fontAdapter.getFont(), block, position,
                    selection != null ? selection.getRectangle() : null));
                e.consume();
                return;
            }
        }
