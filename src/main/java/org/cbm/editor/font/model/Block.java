...
public class Block
{
	...
	public Block pack()
	{
		...
		if ((minimumX > 0) || (minimumY > 0) || (maximumX < getWidthInCharacters())
				|| (maximumY < getHeightInCharacters()))
		{
			resize(minimumX, minimumY, maximumX - minimumX, maximumY - minimumY);
		}
			...
			final int heightInCharacters)
	{
		...
		try
		{
			if ((characterX != 0) || (characterY != 0) || (this.widthInCharacters != widthInCharacters)
					|| (this.heightInCharacters != heightInCharacters))
			{
	...
	public void defenseCharacterXY(final int characterX, final int characterY)
	{
		...
		{
			throw new IllegalArgumentException(String.format("Character position out of bounds: %d, %d", characterX,
					characterY));
		}
	}
